...
nodes. For instance, a \ac{SMR} protocol like
...
system resilient to catastrophic full system shutdowns. From a
formal point of view, however, these systems are treated in a
very similar fashion to others which use volatile state only
replicas. Indeed, protocols which explicitely consider the
persistent aspect of the nodes focus on when and what data they need to
persist so that, in the event of a restart, the node can
operate as if nothing happened.

Although this approach is sufficient for the general use case, in
