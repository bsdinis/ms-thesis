...
\rr{Devias separar em duas secçoes: uma só de Asymmetric
Synchronization (com os princípios que se aplicam a qualquer
system design), e outra com o R2-S2 design com esta instância em
particular}
...
sync set} from now on. \rr{quite subtle why should we care about
replicas in the synch set but not in the write quorum. If they
are not in the write quorum then why command them to synch? Is
this a performance optimization in case other synch replicas are
slow to reply?}\bsd{it is a safety consideration: we need to
guarantee that we get a sync reply. a strawman alternative would
be to only send $M_R$ non-sync requests plus $W_Q - M_R$ sync
requests; then, we would wait for the write quorum and send
requests to the other replicas if needed. This would force us to make a timing
assumption and possibly get high tail latencies}

\paragraph{Achieving strong persistence.} To achieve strong
persistence, it is sufficient that the critical sync set is
never null \rr{this is stated in too strong terms: if it is a
single replica then it can have a permanent fault and this could
imply data loss. Maybe you can restate it in a way that is
broader, saying "a small subset of replicas (maybe even a single
replica, assuming the likelihood of that replica having a
permanent data loss shortly after the operation is
small)"}\bsd{our model does not really deal with permanent data
losses\ldots if the replica in the critical sync set suffers a
permanent failure, then the number of rollbacks is $M_R + 1$ and
we can't really do anything, right?}. This implies that there is always at least one
...
very difficult read. You should expand and rethink the structure}
...
reply, batch and wait or sync to log\rr{qual a diferença dos 2
últimos? Pensava que eram só duas opções? Confuso}) to be applied by replica
...
strong persistence.\rr{creio que não definiste nunca "strong
persitency". Acho que aqui deves antes dizer que obedece às
condições do modelo RR}\bsd{não ficou definida no R2-S2 Design?}
...
on.\rr{Talvez valha a pena elaborar aqui, com uma figura que dê
um exemplo do batch size que consegues obter graças a este
schedule}\bsd{não percebi bem. contudo, posso fazer uma time
series dos schedules, e mostrar que no constant schedule há
replicas que nunca fazem uma batch}
...
\rr{A sucessão seguinte de parágrafos muito curtos é um pouco telegráfica.
Para além de tentar expandir um pouco, nesta transiçao podes enquadrar o que vem de seguida:
"With these data structures in place, the design of the various
different operations -- namely reads, writes that need to be
synched (batch and wait), and writes that can be synched in the
background (batch and reply) -- becomes quite simple.}
...
\rr{Está um pouco telegráfico
Aqui justifica-se explicar que podes para começar e para
simplificar assumir que as falhas são independentes em todas as
réplicas, e daí derivar as fórmulas abaixo com mais calma. Depois
podes fazer estas considerações num parágrafo ou subsecção
separada.}
