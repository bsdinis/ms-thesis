...
\acp{TEE}. Consider a replicated key value store, where each node
maintains a local key value store. When writing to the local key
...
\begin{figure}[t]
    ...
    \caption{Example of a \new{partially intersecting} \ac{RR} quorum system ($M_R=4$,
...
literature, the intersection of quorums is key property, but
...
\begin{defin} [Fully Intersecting \ac{RR} Quorum System]
    A \ac{RR} Quorum System in which there for all pairs of read
...
\bsd{point forward about preemptive and cautious approaches?}
}
...
with the data they store. \new{This clears any suspicion that
...
\ac{RR} model presents a subtle challenge \new{in the context of
...
\new{This mechanism can be altogether skipped if the quorum
...
has concluded (\textbf{I1}). \new{As was the case in the register
...
the \ac{RR} quorum systems and associated protocols, thus
generalizing the crash fault model. It is interesting to consider
whether it can itself be further generalized.
...
suspicion function being a bad abstraction for capturing
intermediate fault models between the crash and Byzantine.
}
