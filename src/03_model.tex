...
for replicated \acp{TEE} with persistent state. Subsequently
show how this fault model can also be applied in the context of
replicated storage systems with persistent storage that operate
outside of the adversarial model of \acp{TEE}, where \ac{CFT} is sufficient for
\emph{correctness} but \ac{RR} can be leveraged for
\emph{performance}. Then, we derive requirements for quorum systems in the
...
considered to suffer only crash faults if the codebase is trusted (as opposed to Byzantine
