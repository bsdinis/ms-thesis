...
Nodes in the \ac{RR} model are \ac{TEE} instances with external
...
%PD: following doesn't belong into the definition of the fault model
%Nodes have the option of running a verification protocol, through
%which they can either ensure that their state is not stale, and
%therefore clear any suspicion of being rolled back, or discover that
%part of their state is stale, and run a recovery protocol to fix it
%(or simply wait for it to be overwritten).
...
%Our replicated \ac{TEE} system model defines a message-passing system, consisting of client machines and a group of $N$ \ac{TEE}-encapsulated processes (\emph{replicas}).
...
%\bsd{took a stab at introducing $s$ differently, as Deepak raised a lot of questions about it}
%Our model requires that, in each instance of the quorum RPC pattern,
%each replica must report whether it suspects that its state was rolled back. The client that gathers the
%replies from the replicas then must count the total number
%of these suspicions, and we refer to this value as a per-RPC
%variable $s$, $0 \le s \le N$.
...
Note that $s$ and $M_R$ are different
...
\begin{property}[Operational Liveness]
    ...
    it.
%  read or write
%  operations, i.e., whether the system has write liveness or read
%  liveness.
\end{property}
...
%\rr{In the following discussion, I think that in addition to claiming that this gives us good performance, we should also explain that this follows naturally from the properties of \acp{TEE}, which, unlike in traditional replicas, have a dynamic nature due to each replica determining, at runtime, if it can be certain that its state is up-to-date or not.}
%\rr{Also in the following discussion, the discussion about the parameters has some redundancy with the last paragraph of the opening section of this section. Can we integrate the two?}
...
%% However, it is worthwhile to consider how $R$ and $F$ affect
%% the properties of the system. $R$ encodes the dimension of an
%% attack on the system. If this parameter is exceeded (i.e.\ more
%% than $R$ replicas are rolled back), the system loses its
%% freshness guarantees. $F$ determines the availability of the
%% system: if more than $F$ replicas are crashed at any point of
%% time, the system loses its liveness.
...
%, or by proposes a weighted quorum, where varying weights are assigned to each
%replica, in such a way that more weight can be ascribed to replicas
%with better response time, thus improving performance.  Similarly,
%Grid quorums~\cite{grid_quorums} arrange nodes in a grid and contact a
%row for read operations and a column for write operations leading to a
%quorum size that is asymptotically more efficient in the number of
%replicas, albeit at a larger replication cost: ${(F + 1)}^2$ replicas
%for \ac{CFT}, instead of $2F + 1$.
...
%instead of being a means to achieve a certain goal as in prior research.
...
straightforward.
% , and proving that the modified versions comply with linearizability.

\subsection{Principles and challenges for protocol adaptation}

% 1. Starting point is crash - because behavior is correct
When adapting existing protocols to the \ac{RR} model, it is
convenient to start from a crash fault tolerant protocol.
%and not a protocol for an adversarial model such as \ac{BFT}.
...
% 2. Most important aspect is quorum adaptation
% PD: How the nodes communicate doesn't seem relevant
%Most distributed replication protocols follow a structure where
%communication is conducted through a primitive called a ``quorum
%RPC''~\cite{Malkhi:Reiter:BQS:98,lorenzo:framework}. This consists of
%a client-driven communication with an RPC pattern, where the client
%\update{(or a replicas acting as a client of the RPC primitive)}
%contacts all replicas in a replica group, and waits for a quorum of
%replies.
...
accordingly.
%, including the necessary adaptation to convey the boolean value corresponding to the suspicion status in each replica's reply.

% 3. suspicion and recovery
...
in the crash model.
%since a restart in that model would be equivalent, e.g., to a network
%partition that leads to a long period of message loss.
...
%% Handling asymmetric quorums is more intricate. The main issue is
%% that read quorums do not necessarily intersect, making it
%% possible to have clients reading different values depending on
%% which quorum they contact, thus breaking consistency. Note that
%% this \emph{split-brain} syndrom is not a \emph{direct}
%% consequence of the fault model, but rather of the resulting
%% quorum system which is naturally asymmetric. The techniques used
%% to solve this problem are generalizable to asymmetric
%% non-intersecting quorums in crash fault tolerant protocols.
...
%in a more intricate part of the proof,
...
%First, we explain this normal case protocol, adapted to use
%\ac{RR} quorums,
%and then we highlight how we extended the protocol with a read-only
%optimization to leverage the properties of the
%\ac{RR} model.


%% n this section is to adapt \emph{one}
%% existing Paxos variant to showcase the changes required by the
%% \ac{RR} model. To this end, we chose a
%% Consequently, we use as a starting point a
%% variant focused on simplicity: \emph{Paxos for System
%% Builders}~\cite{paxos_builders}. Although leader election and
%% view changes are important and intricate parts of any
%% Paxos-related algorithm, we do not approach them, because the
%% \ac{RR} model has no impact on them and they add
%% considerable complexity to the exposition. This way, the focus is
%% kept on the fault model.
...
%final protocol step gathers the largest of a read and write
%quorum, where the larger one can be of either type, depending on
%the number of messages that are marked as suspected.
%Once a slot is marked as accepted and all previous slots have
%been executed, the leader executes the operation and returns the
...
%the internal operation log where all operations are serialized.
...
%(or, more precisely, current state that reflects the output of an operation)
...
%% Note that in the full proof we should also prove uniqueness of ts for writes
